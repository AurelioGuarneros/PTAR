...
for pin in pwm_pins + [relay_pin, RELE_ELECTROVALVULA_PIN]:
    GPIO.setup(pin, GPIO.OUT)
    GPIO.output(pin, GPIO.LOW if pin != relay_pin else GPIO.HIGH)
...
def pwm_loop():
    global running
    pin1, pin2 = pwm_pins[0], pwm_pins[1]  # Definir pines correctamente
    
    while running:
        periodo = 1.0 / frecuencia
        tiempo_encendido = (duty_cycle / 100) * periodo
        tiempo_apagado = max(0, periodo - tiempo_encendido - (2 * dead_time))  # Evitar negativos
...
def actualizar_pwm():
    global frecuencia, duty_cycle, dead_time, running
    ...
    try:
        ...
        if not (0 <= duty_cycle <= 100):
            raise ValueError("Duty cycle fuera de rango")
        
        if running:
            running = False  # Detener el hilo actual
            time.sleep(0.1)  # Esperar para evitar conflictos
...
def actualizar_barra():
    ...
    presion = max(3.5, min(presion, 9.26))
...
dead_time_entry = tk.StringVar(value="0.0005")
...
ttk.Label(left_frame, text="Tiempo Muerto (s):").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
ttk.Entry(left_frame, textvariable=dead_time_entry, width=10).grid(row=1, column=1, padx=5, pady=5)
ttk.Label(left_frame, text="Ciclo de trabajo (%)").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
ttk.Entry(left_frame, textvariable=duty_entry, width=10).grid(row=2, column=1, padx=5, pady=5)
ttk.Label(left_frame, text="Tiempo (min)").grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
ttk.Entry(left_frame, textvariable=time_entry, width=10).grid(row=3, column=1, padx=5, pady=5)
...
ttk.Button(left_frame, text="Iniciar PWM", command=actualizar_pwm).grid(row=3, column=0, columnspan=2, pady=10)
